...
Last Updated: 09-21-26 00:28 · Revision 1.7
...
**Released as v1.3.0 on 09-21-26.** The close-out named the next release as what would put this in front of users, and it followed the same night: the release commit `f2e0d0e` (version 1.3.0, the README's status line), continuous integration green in all five jobs, the annotated tag `v1.3.0` carrying the release notes, the GitHub release with `Snapmockit-1.3.0-x86_64.AppImage` and `Snapmockit-1.3.0-x86_64.flatpak`, and the wheel and source distribution on the Python Package Index at 00:27 on Doug's approval of the `pypi` environment, attested to `dbower44022/snapmockit` and `ci.yml`. **The two display checks this work owes are now owed on the released files**: Help > Add to Menu from the released AppImage, and from a `pipx install snapmockit` of 1.3.0.

**What of end-to-end pass finding 1 remains: nothing of the finding, one thing of its delivery.** The finding asked for an action that installs the entry and icons the AppImage carries and removes them on request; that exists and has been run on the display. What remained was that no released file carried it; v1.3.0, above, does. The display check on those released files is the last thing this work owes.
...
|---|---|---|---|
| 1.7 | 09-21-26 00:28 | Claude (Claude Code) | Released as **v1.3.0** on 09-21-26, the same night as the close-out (Section 8): tag `v1.3.0` on `f2e0d0e`, both Linux files on the GitHub release, and the wheel and source distribution on the Python Package Index at 00:27 on Doug's approval. What this work owes is now two display checks on the released files. |
