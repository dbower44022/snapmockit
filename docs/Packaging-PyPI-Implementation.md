...
Last Updated: 09-18-26 16:43 · Revision 1.0
...
|---|---|---|---|
| 1 | The decisions, the metadata, the command, the sdist's contents, proven here | In progress: decisions taken 09-18-26 (Section 2) | this commit |
...
6. `launch_command` in `config/packaging.py` already answers `snapmockit` wherever that name is on the path, so the shortcut command of Phase 2 needs a test for the index's form, not new code.
