...
Last Updated: 09-12-26 14:51 · Revision 1.1
...
1. `docs/Basic-Shape-Shared-Drawing-Implementation.md` (revision 1.4 or later), whole. It is this work's own notes: Section 1's phase table says where to start, Section 2 carries the four decisions and eight silences and three corrections to the first kickoff, Sections 3 to 5 are Phase 1, Section 6 is Phase 2 with its five silences and its measured cost per Shift move, Section 7 is what settled between the phases, and Section 8 is Doug's display run.
...
4. `PRDs/SnapMock-Basic-Shape-Annotation-Tools-PRD.html` (version 1.16): Section 2 whole, then the Status Bar Hints tables 3.7, 4.8, 5.7, 6.7, 7.7, 8.7, and 9.11, the Shared Behavior rows of Section 12, and the 1.13 to 1.16 revision rows, which are this work's own.
5. `PRDs/SnapMock-General-UI-PRD.html` (version 2.31): 1.3, 5.2, 6.6, Section 9, 12.2, and 15.1, whose overflow behaviour changed on 09-12-26 in another session. `PRDs/SnapMock-Technical-Architecture-PRD.html` (version 1.39): 3.3, 3.9, 8, and 10.
...
## Starting state, verified at commit 7ab3bf2 on 09-12-26

This replaces the first kickoff's "Starting state" section. Its three probes are closed: a locked or hidden layer now refuses the drag, a tool switch during a drag leaves nothing in the scene, and Shift and the centre-draw modifier both work on the Rectangle and the Ellipse. **All three are confirmed on a real display**, along with four more, in the run of 09-12-26 recorded as Section 8 of this work's notes: ten checks, ten as described, none failed.
...
**Phases 1 and 2 owe no display check.** Doug ran seven on 09-12-26 and all seven passed: Shift squaring the rectangle and circling the ellipse, Ctrl drawing from the centre and Shift with Ctrl giving a centred square, a Freehand stroke mixing a curve and a Shift-held straight run, Escape dropping a drag with the button still down, a tool switch during a drag leaving nothing, and a locked layer refusing the drag with its message. The two the other session owed — the colour picker's Saved row and the Tool Options Bar's "More…" popover — passed in the same sitting. Section 8 of this work's notes records all of it, including what the run deliberately did not settle.

**The display checks Phases 3 to 5 will owe**: the dimension tooltip at each of the seven tools, the tooltip near a viewport edge, the guide lines to the rulers, the preview at 70 percent opacity, and three shapes drawn in succession without touching anything between them. Still owed from other works and not this one's: a blur region in Solid Fill — worth re-running, since the fault was narrowed to the Blur tool's own Fill swatch and the colour picker was rebuilt on 09-12-26, though the run above did not test that swatch — a highlight in Multiply over dark text, the painted and erased blur regions, the Whole Layer region and its source modes, and the three the Eyedropper and Blur performance work added.
...
|---|---|---|---|
| 1.1 | 09-12-26 14:51 | Claude (Claude Code) | Doug's display run of 09-12-26 folded in: Phases 1 and 2 owe no display check, the two the other session owed are answered, and the starting state is restated at commit 7ab3bf2. The list of checks the remaining phases will owe is separated from what other works still owe, with the Solid Fill check named as worth re-running. |
