...
Last Updated: 09-15-26 01:34 · Revision 1.9
...
Two notes Doug left. On the Enter that ends brush editing: "It remains the blur tool." Headlessly, after the same Enter the session is over, the viewport's cursor is the arrow, the active tool is the Select tool, and the status bar reads the Select tool's idle line; not reproduced. **A retest is written and owed:** section 10 of the checklist page `SnapMock Blur and Eyedropper Display Checks` (artifact b19bdd2e) looks at the pointer, the status bar, and the palette highlight before, during, and after brush editing, so the one that stays can be named; it had not been run when the session that wrote it closed on 09-15-26. On New Layer (Ctrl+Shift+N): "Layer 1 remained active." Reproduced: `AddLayerCommand` adds the layer above the active one and leaves the active layer where it was, and the Layers panel's highlight stays with it; General UI PRD Section 7 says only "create a new empty annotation layer above the active layer" and is silent on activation. Raised with Doug and decided on 09-14-26: a new layer becomes the active layer (General UI PRD 2.36; General UI notes Section 27).

## Change Log

| Rev | Date (MM-DD-YY HH:MM) | Author | Change |
|---|---|---|---|
| 1.9 | 09-15-26 01:34 | Claude (Claude Code) | Section 12: the brush-editing retest written as section 10 of the checklist page and still owed. |
| 1.8 | 09-14-26 09:50 | Claude (Claude Code) | Section 12: the New Layer silence decided, with the pointer to General UI PRD 2.36. |
