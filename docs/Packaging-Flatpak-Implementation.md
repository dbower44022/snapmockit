...
**Finding 7, 09-18-26 00:03: a click in the thumbnail grid selected nothing the panel's actions could see — the real cause of finding 6.** Doug retried against the fixed bundle and got the same answer, from a plain left-click and then Delete. Reproduced here with a real mouse click on the offscreen platform: in the **grid** the click leaves `selectedIndexes` holding `(row 0, column 0)` and `selectedRows(COL_NAME)` **empty**, because `selectedRows` counts a row only when every one of the model's four columns is selected; in the **list**, which selects whole rows, both answer. `selected_paths` read `selectedRows`, so in the grid — the default view — every action that works on the selection (Delete, Rename, Copy, Export, Move to Folder, Properties) answered "select a file".

Why the context menu sometimes worked: it selects the file under the cursor with the Rows flag, but only when that file is **not already selected**. A right-click on a file just clicked was therefore dead, and a right-click on an untouched file worked. That is exactly the difference between Doug's run and the tests, which selected through `select_path` and its Rows flag.

**Fixed in `ui/library_panel.py`:** `selected_paths` reads `selectedIndexes`, keeps one index per row in row order, and answers for either view. Four tests drive a real click in the grid and in the list and then delete through the panel. Finding 6's fix stands on its own — a reload still must not lose the selection — but this is what Doug was hitting. **A defect of the application, in every form:** the AppImage and the source have it too, and it has been there since the Library panel's two views were built. **The suite at 4a1bb2e:** 1760 passed, 14 skipped, 1 deselected, in 6 minutes 9 seconds. **Proven on Doug's display, 09-18-26 00:05:** he deleted five captures from the grid after a plain click, and the machine shows them in the library's session trash under the running instance's folder, the library down to 40 files; the system trash receives them when the application closes.

### 11.2 Section 12 and the close-out of Phase 2, 09-18-26 00:16

**The delete fix passed on the display.** Doug deleted from the thumbnail grid after a plain click, quit, and found the files in his own trash. Checked on the machine afterwards: **44 captures are in `~/.local/share/Trash/files`**, the library's `.trash` is empty, `~/.var/app/io.github.dbower44022.snapmockit/data/Trash` does not exist, and no Snapmockit process is left. Correction 5.2 is therefore proven end to end on the display: inside a sandbox the files reach the user's own trash through the desktop's trash service, and nothing lands in the trash folder the sandbox would otherwise use. Doug's library is down to two files, and every deleted capture is recoverable from the trash.

**Measured on this machine, against Technical Architecture PRD Section 8.** Startup from `flatpak run` to the main window mapped on the X11 display, polled at 50 ms with `wmctrl`, three runs: **1.13 s, 0.98 s, 0.97 s**, against the target of under two seconds, and against the AppImage's 1.63 to 1.70 s from the sealed file. The Flatpak starts faster because there is no file system to mount and the runtime's Qt is already on the machine. **Sizes:** the bundle is 25.4 MiB (26 MB as the file manager counts), and the installed application is 134 MB beside the shared runtime, against the AppImage's 122.3 MB standing alone.
